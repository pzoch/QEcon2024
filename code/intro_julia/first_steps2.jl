...
first_functionA(3,2)
# You can also define simple functions this way:
first_functionB(x,y) = x*y 
first_functionB(3,2)

#You can define a new function which will use other function but ALWAYS evaluate it at particular value
second_function(x) = first_functionB(x,3)

#10 becomes a default value for y, though if you want you can still change it upon calling the function
...
#Sometimes it is usefuul to have patameters defined as KEYWORD arguments
function fifth_function(x;a,b)
    return a*x+b
end

fifth_function(2,2,1) #ERROR!

fifth_function(2,a=2,b=1)

#You can combine both a keyword arguments and default valuse:
function sixth_function(x;a=2,b=1)
    return a*x+b
end

sixth_function(2,a=2,b=1) #ERROR!
sixth_function(2)

#You can use either way to define a "squared" function:
function squared(x)
    return x^2
end
squared(x) = x^2
...
function eight_function(x)
    a = x^2
    b = 2 * a
    return (; a, b)  
end
solution = eight_function(2)
solution.a

(; a, b)  = eight_function(2)
...
#############################        QUICK TASK:         ############################# 
# Write a function which takes a number (n) as an argument and returns a mean of values 1,2,3,...,n.
# 1. Define a function my_mean(n)
# 2. Define a variable mean=0
# 4. Use a for loop to get a sum of numbers from 1 to n
# 5. Then use calculated sum to get a mean
# 6. Return a mean
# 7. Test a function
####################################################################################### 
